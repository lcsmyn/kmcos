...
integer(kind=iint), parameter, public :: CO2 = 1
integer(kind=iint), parameter, public :: COOH = 2
integer(kind=iint), parameter, public :: empty = 3
integer(kind=iint), public :: default_species = empty


! Process constants

integer(kind=iint), parameter, public :: CO2_COOH = 1
integer(kind=iint), parameter, public :: COOH_CO = 2
integer(kind=iint), parameter, public :: CO_de = 3


integer(kind=iint), parameter, public :: representation_length = 133
...
integer(kind=iint), parameter, public :: nr_of_proc = 3
...
!******
    ...
    if (.not. no_banner) then
        print *, "+------------------------------------------------------------+"
        print *, "|                                                            |"
        print *, "| This kMC Model 'CO2 Reduction to CO (with salt precipitation)' was written by |"
        print *, "|                                                            |"
        print *, "|             Lucas Yuan (lucasmyuan@gmail.com)              |"
        print *, "|                                                            |"
    ...
    do k = 0, system_size(3)-1
        do j = 0, system_size(2)-1
            do i = 0, system_size(1)-1
                ...
                select case(layer)
                case (simple_cubic)
                    call replace_species((/i, j, k, simple_cubic_Ag_site/), null_species, empty)
                end select
            end do
        end do
    end do
    ...
    do k = 0, system_size(3)-1
        do j = 0, system_size(2)-1
            do i = 0, system_size(1)-1
                select case(layer)
                case(simple_cubic)
                    call touchup_simple_cubic_Ag_site((/i, j, k, simple_cubic_Ag_site/))
                end select
            end do
        end do
    end do
...
!******
    ...
    select case(proc)
    case(CO2_COOH)
        call put_COOH_simple_cubic_Ag_site(lsite)

    case(COOH_CO)
        call take_COOH_simple_cubic_Ag_site(lsite)
        call put_CO_simple_cubic_Ag_site(lsite)

    case(CO_de)
        call take_CO_simple_cubic_Ag_site(lsite)

    end select

end subroutine run_proc_nr

subroutine put_CO_simple_cubic_Ag_site(site)
    ...
    if(avail_sites(CO2_COOH, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(CO2_COOH, site)
    endif

    ! enable affected processes
    call add_proc(CO_de, site)

end subroutine put_CO_simple_cubic_Ag_site

subroutine take_CO_simple_cubic_Ag_site(site)
    ...
    ! disable affected processes
    if(avail_sites(CO_de, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(CO_de, site)
    endif

    ! enable affected processes
    call add_proc(CO2_COOH, site)

end subroutine take_CO_simple_cubic_Ag_site

subroutine put_CO2_simple_cubic_Ag_site(site)

    integer(kind=iint), dimension(4), intent(in) :: site

    ! update lattice
    call replace_species(site, empty, CO2)
...
end subroutine put_CO2_simple_cubic_Ag_site

subroutine take_CO2_simple_cubic_Ag_site(site)

    integer(kind=iint), dimension(4), intent(in) :: site

    ! update lattice
    call replace_species(site, CO2, empty)

    ! enable affected processes
    call add_proc(CO2_COOH, site)

end subroutine take_CO2_simple_cubic_Ag_site

subroutine put_COOH_simple_cubic_Ag_site(site)

    integer(kind=iint), dimension(4), intent(in) :: site

    ! update lattice
    call replace_species(site, empty, COOH)
    ...
    ! enable affected processes
    call add_proc(COOH_CO, site)

end subroutine put_COOH_simple_cubic_Ag_site

subroutine take_COOH_simple_cubic_Ag_site(site)

    integer(kind=iint), dimension(4), intent(in) :: site

    ! update lattice
    call replace_species(site, COOH, empty)

    ! disable affected processes
    if(avail_sites(COOH_CO, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(COOH_CO, site)
    endif

    ! enable affected processes
    call add_proc(CO2_COOH, site)

end subroutine take_COOH_simple_cubic_Ag_site

subroutine touchup_simple_cubic_Ag_site(site)

    integer(kind=iint), dimension(4), intent(in) :: site

    if (can_do(CO2_COOH, site)) then
        call del_proc(CO2_COOH, site)
    endif
    if (can_do(COOH_CO, site)) then
        call del_proc(COOH_CO, site)
    endif
    if (can_do(CO_de, site)) then
        call del_proc(CO_de, site)
    endif
...
end subroutine touchup_simple_cubic_Ag_site
